...
static void window_cheats_invalidate(rct_window *w)
{
	...
	// Current weather
	int currentWeather = gClimateCurrentWeather;
	if (currentWeather == WEATHER_SNOW) { window_cheats_misc_widgets[WIDX_WEATHER].image = STR_SNOWING; }
	else { window_cheats_misc_widgets[WIDX_WEATHER].image = STR_SUNNY + currentWeather; }
}
