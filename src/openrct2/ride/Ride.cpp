...
void ride_breakdown_add_news_item(Ride* ride)
{
    ...
    {
        news_item_add_to_queue(News::Item::Type::Ride, STR_RIDE_IS_BROKEN_DOWN, ride->id);
    }
}
...
static void ride_breakdown_status_update(Ride* ride)
{
    ...
    if (ride->lifecycle_flags & RIDE_LIFECYCLE_BROKEN_DOWN)
    {
            ...
            && ride->mechanic_status != RIDE_MECHANIC_STATUS_HAS_FIXED_STATION_BRAKES)
        {
            ...
            {
                news_item_add_to_queue(News::Item::Type::Ride, STR_RIDE_IS_STILL_NOT_FIXED, ride->id);
            }
        }
    }
...
static void ride_entrance_exit_connected(Ride* ride)
{
    for (int32_t i = 0; i < MAX_STATIONS; ++i)
    {
        ...
        if (!entrance.isNull() && !ride_entrance_exit_is_reachable(entrance))
        {
            ...
            if (gConfigNotifications.ride_warnings)
            {
                news_item_add_to_queue(News::Item::Type::Ride, STR_ENTRANCE_NOT_CONNECTED, ride->id);
            }
        ...
        if (!exit.isNull() && !ride_entrance_exit_is_reachable(exit))
        {
            ...
            if (gConfigNotifications.ride_warnings)
            {
                news_item_add_to_queue(News::Item::Type::Ride, STR_EXIT_NOT_CONNECTED, ride->id);
            }
            ride->connected_message_throttle = 3;
        }
    }
...
void Ride::Crash(uint8_t vehicleIndex)
{
    ...
    {
        news_item_add_to_queue(News::Item::Type::Ride, STR_RIDE_HAS_CRASHED, id);
    }
}
