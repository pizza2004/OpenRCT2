    ...
    if (researchItem->type == Research::EntryType::Ride)
    {
        ...
        if (rideEntry != nullptr && base_ride_type != RIDE_TYPE_NULL)
        {
            ...
            if (!gSilentResearch)
            {
                ...
                {
                    news_item_add_to_queue(News::Item::Type::Research, availabilityString, researchItem->rawValue);
                }
            }
            ...
            if (!gSilentResearch)
            {
                if (gConfigNotifications.ride_researched)
                {
                    news_item_add_to_queue(
                        News::Item::Type::Research, STR_NEWS_ITEM_RESEARCH_NEW_SCENERY_SET_AVAILABLE, researchItem->rawValue);
                }
            }
