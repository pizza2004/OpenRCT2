...
static void scenario_entrance_fee_too_high_check()
{
    ...
    if ((gParkFlags & PARK_FLAGS_PARK_OPEN) && park_get_entrance_fee() > max_fee)
    {
        if (!gParkEntrances.empty())
        {
            ...
            {
                NewsItem::AddToQueue(NewsItem::Type::Blank, STR_ENTRANCE_FEE_TOO_HI, packed_xy);
            }
        }
    }
}
...
static void scenario_objective_check_guests_and_rating()
{
    if (gParkRating < 700 && gDateMonthsElapsed >= 1)
    {
        ...
        if (gScenarioParkRatingWarningDays == 1)
        {
            ...
            {
                NewsItem::AddToQueue(NewsItem::Type::Graph, STR_PARK_RATING_WARNING_4_WEEKS_REMAINING, 0);
            }
        }
        else if (gScenarioParkRatingWarningDays == 8)
        {
            if (gConfigNotifications.park_rating_warnings)
            {
                NewsItem::AddToQueue(NewsItem::Type::Graph, STR_PARK_RATING_WARNING_3_WEEKS_REMAINING, 0);
            }
        }
        else if (gScenarioParkRatingWarningDays == 15)
        {
            if (gConfigNotifications.park_rating_warnings)
            {
                NewsItem::AddToQueue(NewsItem::Type::Graph, STR_PARK_RATING_WARNING_2_WEEKS_REMAINING, 0);
            }
        }
        else if (gScenarioParkRatingWarningDays == 22)
        {
            if (gConfigNotifications.park_rating_warnings)
            {
                NewsItem::AddToQueue(NewsItem::Type::Graph, STR_PARK_RATING_WARNING_1_WEEK_REMAINING, 0);
            }
        }
        else if (gScenarioParkRatingWarningDays == 29)
        {
            NewsItem::AddToQueue(NewsItem::Type::Graph, STR_PARK_HAS_BEEN_CLOSED_DOWN, 0);
            gParkFlags &= ~PARK_FLAGS_PARK_OPEN;
            scenario_failure();
