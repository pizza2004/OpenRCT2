...
static void window_news_scrollmousedown(rct_window* w, int32_t scrollIndex, const ScreenCoordsXY& screenCoords)
{
    ...
    for (const auto& newsItem : gNewsItems.GetArchived())
    {
        if (mutableScreenCoords.y < itemHeight)
        {
            ...
            else if (mutableScreenCoords.x < 351 && newsItem.GetTypeProperties() & NEWS_TYPE_HAS_SUBJECT)
            {
                buttonIndex = 1;
                break;
            }
            else if (mutableScreenCoords.x < 376 && newsItem.GetTypeProperties() & NEWS_TYPE_HAS_LOCATION)
            {
                buttonIndex = 2;
                break;
            }
...
static void window_news_scrollpaint(rct_window* w, rct_drawpixelinfo* dpi, int32_t scrollIndex)
{
    ...
    for (const auto& newsItem : gNewsItems.GetArchived())
    {
        ...
        // Subject button
        if ((newsItem.GetTypeProperties() & NEWS_TYPE_HAS_SUBJECT) && !(newsItem.Flags & NEWS_FLAG_HAS_BUTTON))
        {
        ...
        // Location button
        if ((newsItem.GetTypeProperties() & NEWS_TYPE_HAS_LOCATION) && !(newsItem.Flags & NEWS_FLAG_HAS_BUTTON))
        {
